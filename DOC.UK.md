...
- [Логер у контексті](#логер-у-контексті)
...
`WithContext`, `FromContext` і `FromContextOr` переносять підготовлений логер
разом із запитом чи задачею, тож рядки глибоко в дереві викликів корелюються з
роботою, яка їх спричинила:

```go
ctx := log.WithContext(r.Context(), requestLogger)

// будь-де нижче, без протягування логера через параметри:
log.FromContextOr(ctx, defaultLogger).Errorf("upstream refused: %v", err)
```

`FromContext` повертає nil, коли нічого не збережено, - ним питають, чи логер
взагалі надали. `FromContextOr` - форма для місця виклику: вона не повертає
nil, хіба що fallback нульовий, тож функції, яка може виконуватись і в
підготовленому контексті, і поза ним, не потрібне розгалуження. Практична
альтернатива цим хелперам - не протягнутий логер, а жодного: рядки пишуться
пакетним логером і пов'язані зі своїм запитом лише часовими мітками.
